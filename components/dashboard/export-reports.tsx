...
export function ExportReports({
  ...
  const generatePDFReport = async (type: "comprehensive" | "executive" = "comprehensive") => {
    ...
    try {
      ...
      // Save the PDF
      const fileName = `IIICI_${type}_Report_${organizationName.replace(/\s+/g, "_")}_${new Date().toISOString().split('T')[0]}.pdf`
      pdf.save(fileName)
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      link.href = url
      link.download = `IIICI_Scores_${organizationName.replace(/\s+/g, "_")}_${new Date().toISOString().split('T')[0]}.csv`
      link.click()
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const link = document.createElement("a")
      link.href = url
      link.download = `IIICI_Excel_Report_${organizationName.replace(/\s+/g, "_")}_${new Date().toISOString().split('T')[0]}.csv`
      link.click()
