    ...
    async session({ session, token }): Promise<any> {
      try {
        if (!session || !session.user) {
          console.error("❌ No session or user found in session callback")
          return null
        }
      ...
      try {
        ...
        if (token?.sub) {
          try {
            ...
            // Don't clear the token on database errors, just log them
            // This prevents infinite re-authentication loops
          }
        }
      } catch (error) {
        console.error("❌ JWT callback error:", error)
        // Return the token as-is to prevent breaking the auth flow
      }
      return token
    },
