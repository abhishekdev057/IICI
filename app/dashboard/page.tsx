...
import {
  RatingDisplay,
  RatingDisplayCompact,
} from "@/components/dashboard/rating-display";
...
const getIndicatorDescription = (indicatorId: string): string => {
  const definitions = {
    "1.1.a":
      "Assesses whether the organization has a formal, written statement defining its innovation intent",
    "1.1.b":
      "Evaluates if the innovation strategy directly supports broader business goals",
    "1.1.c": "Checks for explicit listing of innovation focus areas",
    "1.1.d":
      "Assesses dissemination of innovation intent to employees, partners, and investors",
    "2.1.1":
    ...
    "2.2.1":
      "Measures the extent to which adequate personnel are assigned to support innovation efforts",
    "2.2.2":
    ...
    "3.1.1":
      "Assesses the systematic approach to managing innovation activities",
    "3.2.1":
      "A systematic process is used for the capture, documentation, and tracking of ideas",
    "3.4.1":
    ...
    "4.2.1":
      "How systematic and integrated are the processes for identifying and capturing potential intellectual assets",
    "4.2.3":
    ...
    "4.4.2":
      "How effective and accessible are the internal systems and cultural norms for sharing knowledge, resources, and ideas",
    "5.1.1":
      "How diverse and comprehensive are the sources of strategic intelligence used to inform innovation decisions",
    "5.1.3":
      "How effectively is strategic intelligence integrated into decision-making processes",
    "5.2.2":
    ...
    "6.1.1":
      "To what extent has the organization established and implemented specific metrics for evaluating innovation processes",
    "6.1.2":
      "How effectively is data from these metrics used to support and drive strategic and operational decision-making",
    "6.2.1":
  ...
  return (
    definitions[indicatorId as keyof typeof definitions] ||
...
const getNormalizedScore = (indicatorId: string, value: any): number => {
  ...
  if (measurementUnit.includes("Score")) {
    const maxScore = measurementUnit.includes("(0-3)")
      ? 3
      : measurementUnit.includes("(0-2)")
      ? 2
      : 5;
...
function DashboardContent() {
  ...
  const [currentLoadingSection, setCurrentLoadingSection] =
    useState<string>("");
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        setIsLoading(true);
        setError(null);
        setCurrentLoadingSection("Loading dashboard...");

        // Load basic data first for immediate render
        console.log("🚀 Loading basic dashboard data...");
        const basicResponse = await fetch("/api/dashboard/basic");
        if (!basicResponse.ok) {
          throw new Error(
            `Failed to fetch basic dashboard data: ${basicResponse.status}`
          );
        }

        const basicResult = await basicResponse.json();
        if (basicResult.success) {
          setDashboardData(basicResult.data);
          console.log("✅ Basic dashboard data loaded");
        } else {
          throw new Error(
            basicResult.error || "Failed to load basic dashboard data"
          );
        }

        // Load detailed data in background
        console.log("🔄 Loading detailed dashboard data...");
        setCurrentLoadingSection("Loading detailed data...");

        const detailedResponse = await fetch("/api/dashboard");
        if (detailedResponse.ok) {
          const detailedResult = await detailedResponse.json();
          if (detailedResult.success) {
            setDashboardData(detailedResult.data);
            console.log("✅ Detailed dashboard data loaded");
          }
        } else {
          console.warn(
            "⚠️ Failed to load detailed dashboard data, using basic data"
          );
        }

        setCurrentLoadingSection("Complete");
      } catch (err) {
  ...
  // Loading state - show basic layout immediately
  if (isLoading) {
    return (
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="flex items-center justify-center min-h-[80vh]">
          <Card className="max-w-md">
            ...
            <CardContent>
              <p className="text-muted-foreground mb-4">
                Your assessment is still in draft. Please complete and submit it
  ...
  const displayCertificationLevel =
    getDisplayCertificationLevel(certificationLevel);
  ...
  return (
    <div className="min-h-screen bg-background">
      <Navigation
        variant="dashboard"
        title="IIICI Dashboard"
        showInstitutionBadge
      />
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    {institutionData.organizationSize}
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-amber-700 mt-1">
                    Your assessment is complete! Submit it to receive your
                    official certification and unlock all dashboard features.
                  </p>
                </div>
        ...
        <div className="mb-6 md:mb-8">
          ...
          <p className="text-muted-foreground text-sm md:text-base">
            Track your organization's innovation capability assessment and
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="details" className="space-y-6">
            ...
            {/* Recommendations */}
            {processedDashboardData.recommendations &&
              processedDashboardData.recommendations.length > 0 && (
                <Card>
                  <CardHeader>
                  ...
                  <CardContent>
                    <ul className="space-y-2">
                      {processedDashboardData.recommendations.map(
      ...
      {isLoading && (
        <div className="fixed bottom-6 right-6 z-50">
          <div className="bg-background/95 backdrop-blur-sm border rounded-lg shadow-lg p-3 flex items-center gap-3 min-w-[180px]">
