...
import { pillarTwoIndicators } from "./pillar-two-form-clean"
...
export function PillarTwoFormOrganized() {
  const { state, updateIndicator, updateEvidence, getPillarProgress } = useApplication()
  const [expandedSubPillars, setExpandedSubPillars] = useState<Set<string>>(new Set(["2.1"]))
  
  // Get pillar structure
  const pillarStructure = PILLAR_STRUCTURE.find(p => p.id === 2)
  if (!pillarStructure) return null

  const application = state.application
  if (!application) {
    console.log('🔍 PillarTwoFormOrganized: No application in state');
    return null;
  }

  const pillarData = application.pillarData.pillar_2 || { indicators: {}, evidence: {} }
  
  console.log('🔍 PillarTwoFormOrganized: Received data:', {
    hasApplication: !!application,
    pillarDataKeys: Object.keys(application.pillarData),
    pillar2Data: pillarData,
  ...
  const getSubPillarCompletion = useCallback((subPillarId: string) => {
    ...
    const indicators = pillarTwoIndicators.filter(ind => subPillar.indicators.includes(ind.id))
  ...
  return (
    <div className="space-y-6">
      ...
      {pillarStructure.subPillars.map((subPillar) => {
        ...
        const subPillarIndicators = pillarTwoIndicators.filter(ind => subPillar.indicators.includes(ind.id))
