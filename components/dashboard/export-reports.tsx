...
import { Download, FileText, Table, Share2, Mail, Copy, Printer, ExternalLink, CheckCircle, AlertCircle, Building2, Award, Calendar, User } from "lucide-react"
...
export function ExportReports({
  ...
  const generatePDFReport = async (type: "comprehensive" | "executive" = "comprehensive") => {
    ...
    try {
      ...
      const fileName = `IIICI_Innovation_Assessment_${organizationName.replace(/[^a-zA-Z0-9]/g, "_")}_${new Date().toISOString().split('T')[0]}.pdf`
      ...
      try {
        ...
        toast({
          ...
          description: `${type.charAt(0).toUpperCase() + type.slice(1)} report with company standards has been downloaded.`,
          variant: "default",
        })
      } catch (saveError) {
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const executiveSummaryData = [
        ...
        ["Assessment Date", issuedDate ? new Date(issuedDate).toLocaleDateString('en-US', { 
          year: 'numeric', 
          month: 'long', 
          day: 'numeric' 
        }) : new Date().toLocaleDateString('en-US', { 
        ...
        ["Performance Level", overallScore >= 85 ? "LEADING" : 
                            overallScore >= 70 ? "OPTIMIZING" : 
                            overallScore >= 60 ? "STRUCTURED" : 
                            overallScore >= 50 ? "DEVELOPING" : "INITIATING"],
        [""],
        ...
        ["Strong Pillars (≥70%)", pillarScores.filter(score => score >= 70).length.toString()],
        ["Areas for Improvement", pillarScores.filter(score => score < 70).length.toString()],
        [""],
      ...
      // Add styling information (Excel will interpret some of these)
      executiveSheet['!cols'] = [
        { width: 30 },
        { width: 25 }
      ]
      ...
      const pillarAnalysisData = [
        ...
        [""],
        ["Pillar", "Score (%)", "Status", "Performance Level", "Recommendations"],
        ...pillarScores.map((score, index) => {
          const status = getScoreStatus(score)
          const level = score >= 70 ? "Excellent" : score >= 50 ? "Good" : "Needs Improvement"
          const recommendations = score >= 70 ? "Maintain excellence and share best practices" :
                                   score >= 50 ? "Focus on strengthening weak areas" :
        ...
        [""],
        ["Overall Assessment"],
        ["Average Score", (pillarScores.reduce((a, b) => a + b, 0) / pillarScores.length).toFixed(1) + "%"],
        ["Highest Performing Pillar", pillarNames[pillarScores.indexOf(Math.max(...pillarScores))]],
        ["Lowest Performing Pillar", pillarNames[pillarScores.indexOf(Math.min(...pillarScores))]],
        ["Score Range", `${Math.min(...pillarScores).toFixed(1)}% - ${Math.max(...pillarScores).toFixed(1)}%`]
      ]
      
      const pillarSheet = XLSX.utils.aoa_to_sheet(pillarAnalysisData)
      pillarSheet['!cols'] = [
        { width: 40 },
        { width: 12 },
        { width: 15 },
      ...
      XLSX.utils.book_append_sheet(workbook, pillarSheet, "Pillar Analysis")
      ...
      const recommendationsData = [
        ...
        ["High", "Focus on strengthening areas with scores below 70%", "Significant improvement in overall score", "3-6 months"],
        ["High", "Develop comprehensive action plans for continuous improvement", "Sustainable innovation culture", "6-12 months"],
        ["Medium", "Implement industry best practices and frameworks", "Enhanced innovation processes", "3-9 months"],
        ["Medium", "Establish regular assessment cycles", "Ongoing performance monitoring", "1-3 months"],
        ["Medium", "Invest in innovation training programs", "Improved innovation capabilities", "6-12 months"],
        ["Low", "Create cross-functional innovation teams", "Better collaboration and knowledge sharing", "3-6 months"],
        [""],
      ...
      const rawData = [
        ...
        [""],
        ["Organization", organizationName],
        ["Assessment Date", issuedDate ? new Date(issuedDate).toISOString() : new Date().toISOString()],
      ...
      // Generate Excel file
      const fileName = `IIICI_Innovation_Assessment_${organizationName.replace(/[^a-zA-Z0-9]/g, "_")}_${new Date().toISOString().split('T')[0]}.xlsx`
      ...
      toast({
        ...
        description: "Your comprehensive assessment data has been exported with professional formatting.",
        variant: "default",
      })
        ...
        <html>
          ...
          <body>
            ...
            <div class="org-info">
              ...
              <div class="org-details">
                ...
                <div class="org-detail">
                  ...
                  <span>${issuedDate ? new Date(issuedDate).toLocaleDateString('en-US', { 
                    year: 'numeric', 
                    month: 'long', 
                    day: 'numeric' 
                  }) : new Date().toLocaleDateString('en-US', { 
            ...
            <div class="pillars-section">
              ...
              ${pillarScores.map((score, index) => {
                const statusClass = score >= 70 ? 'excellent' : score >= 50 ? 'good' : 'needs-improvement'
                return `
                  <div class="pillar">
                    <div class="pillar-header">
                      <div class="pillar-name">${pillarNames[index]}</div>
                      <div class="pillar-score ${statusClass}">${score.toFixed(1)}%</div>
                    </div>
                    <div class="pillar-status">Status: ${getScoreStatus(score)} | Level: ${score >= 70 ? "Excellent" : score >= 50 ? "Good" : "Needs Improvement"}</div>
                  </div>
                `
              }).join("")}
            </div>
            ...
            <div class="signature-section">
              ...
              <div class="signature-details">
                ...
                <div class="signature-info">
                  ...
                  <strong>Valid Until:</strong> ${expiryDate ? new Date(expiryDate).toLocaleDateString('en-US', { 
                    year: 'numeric', 
                    month: 'long', 
                    day: 'numeric' 
                  }) : 'N/A'}<br>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Professional reports with company branding, signatures, and detailed analysis
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Multi-sheet Excel reports with professional formatting and comprehensive data analysis
              </p>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 border rounded-lg bg-gradient-to-r from-blue-50 to-blue-100 border-blue-200">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-medium text-blue-900">Professional Comprehensive Report</div>
                  <div className="text-sm text-blue-700">
                    Company-branded PDF with signatures, detailed analysis, and strategic recommendations
                  </div>
                </div>
              </div>
              <Button size="sm" onClick={() => generatePDFReport("comprehensive")} disabled={isGenerating} className="bg-blue-600 hover:bg-blue-700 text-white hover:text-white">
            ...
            <div className="flex items-center justify-between p-4 border rounded-lg bg-gradient-to-r from-green-50 to-green-100 border-green-200">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-medium text-green-900">Executive Summary Report</div>
                  <div className="text-sm text-green-700">
                    High-level overview with company branding for leadership and stakeholders
                  </div>
                </div>
              </div>
              <Button size="sm" variant="outline" onClick={() => generatePDFReport("executive")} disabled={isGenerating} className="border-green-300 text-green-700 hover:bg-green-50">
                <Download className="w-3 h-3 mr-1" />
                Download
              </Button>
            </div>
            ...
            <div className="flex items-center justify-between p-4 border rounded-lg bg-gradient-to-r from-purple-50 to-purple-100 border-purple-200">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-medium text-purple-900">Multi-Sheet Excel Report</div>
                  <div className="text-sm text-purple-700">Professional Excel workbook with 4 comprehensive sheets and detailed analysis</div>
                </div>
              </div>
              <Button size="sm" onClick={exportToExcel} disabled={isExporting} className="bg-purple-600 hover:bg-purple-700 text-white hover:text-white">
                <Download className="w-3 h-3 mr-1" />
                Download
              </Button>
            </div>
            ...
            <div className="flex items-center justify-between p-4 border rounded-lg bg-gradient-to-r from-orange-50 to-orange-100 border-orange-200">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-medium text-orange-900">Print-Ready Report</div>
                  <div className="text-sm text-orange-700">Professional print layout with company branding and signature section</div>
                </div>
              </div>
              <Button size="sm" variant="outline" onClick={printReport} className="border-orange-300 text-orange-700 hover:bg-orange-50">
