...
export function SubPillarDetails({
  pillarId,
  indicators,
  className,
}: SubPillarDetailsProps) {
  const [expandedSubPillars, setExpandedSubPillars] = useState<Set<string>>(
    new Set()
  );
  ...
  const getSubPillarScore = (subPillar: SubPillar) => {
    const subPillarIndicators = indicators.filter((ind) =>
      subPillar.indicators.includes(ind.id)
    );
    if (subPillarIndicators.length === 0) return 0;

    const totalScore = subPillarIndicators.reduce(
      (sum, ind) => sum + ind.normalizedScore,
      0
    );
    // normalizedScore is already in percentage (0-100), so just average it
    return Math.min(totalScore / subPillarIndicators.length, 100);
  };
  ...
  const getSubPillarCompletion = (subPillar: SubPillar) => {
    ...
    const completedIndicators = subPillarIndicators.filter(
      (ind) =>
        ind.rawValue !== null &&
        ind.rawValue !== undefined &&
        ind.rawValue !== ""
    );
    return subPillarIndicators.length > 0
      ? (completedIndicators.length / subPillarIndicators.length) * 100
      : 0;
  };
        ...
        {pillarStructure.subPillars.map((subPillar) => {
          ...
          return (
            <Collapsible
              key={subPillar.id}
              open={isExpanded}
              onOpenChange={() => toggleSubPillar(subPillar.id)}
            >
