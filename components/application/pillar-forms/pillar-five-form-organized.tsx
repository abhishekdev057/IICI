...
import { pillarFiveIndicators } from "./pillar-five-form-clean"
...
export function PillarFiveFormOrganized() {
  const { state, updateIndicator, updateEvidence, getPillarProgress } = useApplication()
  const [expandedSubPillars, setExpandedSubPillars] = useState<Set<string>>(new Set(["5.1"]))
  
  // Get pillar structure
  const pillarStructure = PILLAR_STRUCTURE.find(p => p.id === 5)
  ...
  const getSubPillarCompletion = useCallback((subPillarId: string) => {
    ...
    const indicators = pillarFiveIndicators.filter(ind => subPillar.indicators.includes(ind.id))
  ...
  return (
    <div className="space-y-6">
      ...
      {pillarStructure.subPillars.map((subPillar) => {
        ...
        const subPillarIndicators = pillarFiveIndicators.filter(ind => subPillar.indicators.includes(ind.id))
