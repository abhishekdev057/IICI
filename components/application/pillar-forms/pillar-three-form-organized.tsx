...
import { pillarThreeIndicators } from "./pillar-three-form-clean"
...
export function PillarThreeFormOrganized() {
  const { state, updateIndicator, updateEvidence, getPillarProgress } = useApplication()
  const [expandedSubPillars, setExpandedSubPillars] = useState<Set<string>>(new Set(["3.1"]))
  
  // Get pillar structure
  const pillarStructure = PILLAR_STRUCTURE.find(p => p.id === 3)
  ...
  const getSubPillarCompletion = useCallback((subPillarId: string) => {
    ...
    const indicators = pillarThreeIndicators.filter(ind => subPillar.indicators.includes(ind.id))
  ...
  return (
    <div className="space-y-6">
      ...
      {pillarStructure.subPillars.map((subPillar) => {
        ...
        const subPillarIndicators = pillarThreeIndicators.filter(ind => subPillar.indicators.includes(ind.id))
